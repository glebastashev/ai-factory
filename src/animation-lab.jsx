...
const options = [
  { id: 'flow', name: 'Поток задач', tag: 'Предлагаю начать с этого', description: 'Заявки и документы сходятся в ИИ-модуле. Импульс проходит через систему, на выходе появляются готовые ответы.', emphasis: 'Показывает, что именно делает агентство.', scene: FlowScene },
  { id: 'factory', name: 'ИИ-конвейер', tag: 'Буквально ai-factory', description: 'Задачи заезжают в объёмный модуль и выходят с отметкой готовности. Одна непрерывная производственная линия.', emphasis: 'Связывает название агентства с процессом работы.', scene: FactoryScene },
  { id: 'orbit', name: 'Центр притяжения', tag: 'Акцент на образе', description: 'Световые импульсы движутся по орбитам вокруг объёмного ядра. Продажи, контент и поддержка связаны в одну систему.', emphasis: 'Даёт первому экрану выразительный объём.', scene: OrbitScene },
  { id: 'matrix', name: 'Из данных в действие', tag: 'Графика и типографика', description: 'Разрозненные точки собираются в знак ai, затем превращаются в подтверждение выполненной задачи. Цикл повторяется плавно.', emphasis: 'Ставит в центр переход от задачи к результату.', scene: MatrixScene },
];
